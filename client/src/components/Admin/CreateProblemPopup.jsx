...
const CreateProblemPopup = ({ open, onClose, adminId }) => {
    ...
    const { name, contest } = form;
    const { value: contestId } = contest || {};

    const queryClient = useQueryClient();

    // ✅ Get contests + query meta from context
    const { allContests, allContestsQuery } = useContests() || {};
    const contests = allContests || [];
    const { isLoading, isError } = allContestsQuery || {
        isLoading: false,
        isError: false,
    };
    ...
    return (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white w-[420px] rounded-lg shadow-lg p-4 pb-8">
                ...
                {isError && (
                    <div className="mb-3 text-sm text-red-700 bg-red-50 border border-red-200 px-3 py-2 rounded">
                        Failed to load contests. Please refresh.
                    </div>
                )}
