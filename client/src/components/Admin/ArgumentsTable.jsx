    ...
    <tr className="border-b border-gray-300">
      ...
      {argumentsList.map((arg, idx) => (
        <th key={idx} className="text-left p-3">
          {arg.name}
          <span className="pl-1.5 text-[10px] text-gray-500">({arg.type})</span>
        </th>
      ))}
  ...
  <tr key={testcase._id || index} className="border-t hover:bg-gray-50">
    ...
    {argumentsList.map((arg) => (
      <td key={arg.name} className="p-2.5 text-base">
        {JSON.stringify(testcase.input ? testcase.input[arg.name] : undefined)}
      </td>
    ))}
...
const EditRow = ({
  ...
  <>
      ...
      {argumentsList.map((arg) => (
        <td key={arg.name} className="p-2.5">
          <input
            ...
            onChange={(e) => onChangeValue(row.id, arg.name, e.target.value)}
          />
        </td>
      ))}
    ...
    {errors && <RowError errors={errors} colSpan={argumentsList.length + 3} />}
  </>
);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-sm p-4">
        <h4 className="text-base font-semibold mb-2">Delete testcase?</h4>
...
const ArgumentsTable = ({ argumentsList, problemId }) => {
  ...
  const addInputs = () => {
    ...
    const defaults = Object.fromEntries(
      argumentsList.map((arg) => {
        ...
        return [
          arg.name,
          typeof defVal === "string" ? defVal : JSON.stringify(defVal),
        ];
      }),
    );
  ...
  const addTestcase = (row) => {
    ...
    const inputObject = Object.fromEntries(
      argumentsList.map((arg) => [arg.name, parseJSON(row.values[arg.name])]),
    );
        ...
        <div className="h-full">
          <div className="border border-gray-300 overflow-visible rounded-md">
            <table className="w-full text-sm table-fixed">
              <colgroup>
                <col className="w-12" />
                {[...Array(argumentsList.length + 1)].map((_, idx) => (
                  <col key={idx} />
                ))}
                <col className="w-36" />
                <col className="w-36" />
              ...
              <tbody>
                ...
                {testcases.length === 0 &&
                  ...
                  rows.length === 0 && (
                    <tr>
                      <td
                        colSpan={argumentsList.length + 3}
                ...
                {testcases.map((tc, i) => {
                  ...
                  const editingRow = isEditing ? rows[0] : null;
                  ...
                  if (isEditing && editingRow) {
                    ...
                    return (
                      <EditRow
                        ...
                        onChangeVisibility={updateRowVisibility}
                        onSave={saveEdit}
                        onCancel={cancelEdit}
                        isSaving={updateTestcaseMutation.isPending}
                      />
                    );
                  }
                  ...
                  return (
                    <TestcaseDisplayRow
                      ...
                      argumentsList={argumentsList}
                      onEdit={() => startEdit(i)}
                      onDelete={() => removeTestcase(tc._id)}
                      canEdit={
                        !updateTestcaseMutation.isPending &&
                        !removeTestcaseMutation.isPending
                      }
                      canDelete={!removeTestcaseMutation.isPending}
                    />
                  );
                })}
