...
const flattenOptions = (options) => {
  ...
  options.forEach((item) => {
    if (isGroup(item)) {
      item.options.forEach((opt) => list.push({ ...opt, __group: item.label }));
    } else {
      list.push(item);
    }
  });
...
const CustomSelect = ({
  ...
  const selectedOption = flatOptions.find((opt) => opt.value === value?.value);
  ...
  const handleKeyDown = (e) => {
    ...
    switch (e.key) {
      ...
      case " ":
        e.preventDefault();
        if (open && focusedIndex >= 0) handleSelect(flatOptions[focusedIndex]);
        else setOpen(!open);
        break;
      case "Escape":
        e.preventDefault();
      ...
      case "ArrowUp":
        e.preventDefault();
        if (open) setFocusedIndex((prev) => (prev > 0 ? prev - 1 : prev));
        break;
    }
  };
  ...
  return (
    <div className="w-full h-full relative" ref={selectRef}>
      <button
        ...
        aria-expanded={open}
        className={`${className} flex h-full w-full items-center justify-between rounded-md border text-left shadow-sm transition-all duration-200 outline-none ${padding} ${
          error
