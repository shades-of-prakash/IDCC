...
const UserLogin = () => {
    ...
    // ✅ use running contests from context
    const { runningContests, runningContestsQuery, setSelectedContest } =
        useContests();

    const contests = runningContests || [];
    const isLoading = runningContestsQuery?.isLoading;
    const isError = runningContestsQuery?.isError;
    ...
    const handleStep1Submit = (e) => {
        ...
        if (!formData.contestId)
            newErrors.contestId = "Please select a contest";

        formData.participants.forEach((p, i) => {
            if (!p.name) newErrors[`participant${i}name`] = "Name is required";
            if (!p.regNo)
                newErrors[`participant${i}regNo`] = "Reg No is required";
        });
