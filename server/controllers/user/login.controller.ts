...
export const loginUser = async (c: Context) => {
    try {
        ...
        // 🔹 NEW: Block login if this user has already finished the contest
        if (existingSession?.isFinished) {
            return ErrorResponse(
                c,
                "You have already completed this contest. Further login is not allowed.",
                403,
            );
        }
        ...
        if (allUserDetails.length > 0) {
            for (const userDetail of allUserDetails) {
                ...
                if (
                    userDetail.participants.length === 1 &&
                    participants.length === 2
                ) {
                    const existingRegNo =
                        userDetail.participants[0].regNo.toLowerCase();
                    const newOnes = participants.filter(
                        (p) => p.regNo.toLowerCase() !== existingRegNo,
                    );

                    if (newOnes.length === 1) {
                        const existingRegistrations = await UserDetails.find({
                            contestId,
                        });
                        const conflictingEntry = existingRegistrations.find(
                            (u) =>
                                u.participants.some((p) =>
                                    newOnes.some(
                                        (ip) =>
                                            ip.regNo.toLowerCase() ===
                                            p.regNo.toLowerCase(),
                                    ),
                                ),
                        );
            ...
            const conflictingEntry = existingRegistrations.find((u) =>
                u.participants.some((p) =>
                    participants.some(
                        (ip) =>
                            ip.regNo.toLowerCase() === p.regNo.toLowerCase(),
                    ),
                ),
            );
